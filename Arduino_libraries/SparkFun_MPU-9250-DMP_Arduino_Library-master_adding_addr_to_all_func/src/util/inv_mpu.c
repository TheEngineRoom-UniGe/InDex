...
#ifdef AK89xx_SECONDARY
static int setup_compass(bool addr);
 ...
 */
int set_int_enable(unsigned char enable, bool addr)
{
 ...
 *  @return     0 if successful.
 */
int mpu_read_reg(unsigned char reg, unsigned char *data, bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
 ...
 *  @return     0 if successful.
 */
int mpu_init(struct int_param_s *int_param,bool addr)
{ //// TODO check the original code data was not declared here 
     if (addr)
        st.hw->addr = 0x69; 
 else
  st.hw->addr = 0x68;
    unsigned char data[1]; // TODO check the code 
   ...
   st.chip_cfg.accel_half = 0;
    ...
    if (mpu_set_gyro_fsr(2000,addr))
        return -1;
    if (mpu_set_accel_fsr(2,addr))
        return -1;
    if (mpu_set_lpf(42,addr))
        return -1;
    if (mpu_set_sample_rate(50,addr))
        return -1;
    if (mpu_configure_fifo(0,addr))
        return -1;
...
#ifdef AK89xx_SECONDARY
    setup_compass(addr);
    if (mpu_set_compass_sample_rate(10, addr))
        return -1;
#else
    /* Already disabled by setup_compass. */
    if (mpu_set_bypass(0,addr))
        return -1;
#endif

    mpu_set_sensors(0,addr);
    return 0;
}
 ...
 *  @return     0 if successful.
 */
int mpu_lp_accel_mode(unsigned short rate, bool addr)
{
if (addr)
        st.hw->addr = 0x69; 
        else
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_reg(short *data, unsigned long *timestamp,bool addr)
{
       if (addr)
        st.hw->addr = 0x69; 
 else
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_reg(short *data, unsigned long *timestamp, bool addr)
{  
     if (addr)
        st.hw->addr = 0x69; 
 else
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(long *data, unsigned long *timestamp,bool addr)
{
    if (addr)
    st.hw->addr = 0x69; 
    else
 ...
 *  @return     0 if successful.
 */
int mpu_read_6500_accel_bias(long *accel_bias, bool addr) {
    if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
 ...
 *  @return     0 if successful.
 */
int mpu_read_6050_accel_bias(long *accel_bias , bool addr) {
    if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
...
int mpu_read_6500_gyro_bias(long *gyro_bias,  bool addr) {
    if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
 ...
 *  @return     0 if successful.
 */
int mpu_set_gyro_bias_reg(long *gyro_bias , bool addr)
{
    if (addr)
        st.hw->addr = 0x69; 
        else
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias_6050_reg(const long *accel_bias, bool addr) {
    if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
    ...
    if(mpu_read_6050_accel_bias(accel_reg_bias, addr))
        return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias_6500_reg(const long *accel_bias, bool addr) {
   if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
    ...
    if(mpu_read_6500_accel_bias(accel_reg_bias, addr))
        return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_set_gyro_fsr(unsigned short fsr,bool addr)
{   if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_fsr(unsigned char fsr,bool addr)
{ if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
 ...
 *  @return     0 if successful.
 */
int mpu_set_lpf(unsigned short lpf, bool addr)
{  if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
 ...
 *  @return     0 if successful.
 */
int mpu_set_sample_rate(unsigned short rate,bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (st.chip_cfg.lp_accel_mode) {
            if (rate && (rate <= 40)) {
                ...
                mpu_lp_accel_mode(rate,addr);
                return 0;
            }
            /* Requested rate exceeds the allowed frequencies in LP accel mode,
             * switch back to full-power mode.
             */
            mpu_lp_accel_mode(0,addr);
        }
        if (rate < 4)
            rate = 4;
        else if (rate > 1000)
...
#ifdef AK89xx_SECONDARY
        mpu_set_compass_sample_rate(min(st.chip_cfg.compass_sample_rate, MAX_COMPASS_SAMPLE_RATE),addr);
#endif

        /* Automatically set LPF to 1/2 sampling rate. */
        mpu_set_lpf(st.chip_cfg.sample_rate >> 1,addr);
        return 0;
    }
}
 ...
 *  @return     0 if successful.
 */
int mpu_set_compass_sample_rate(unsigned short rate, bool addr)
{ if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
 ...
 *  @return     0 if successful.
 */
int mpu_configure_fifo(unsigned char sensors, bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
  st.hw->addr = 0x68;
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (sensors || st.chip_cfg.lp_accel_mode)
            set_int_enable(1,addr);
        else
            set_int_enable(0,addr);
        if (sensors) {
 ...
 *  @return     0 if successful.
 */
int mpu_set_sensors(unsigned char sensors, bool addr)
{
    if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
    ...
    if (sensors & INV_XYZ_COMPASS)
        mpu_set_bypass(1,addr);
    else
        mpu_set_bypass(0,addr);
#else
 ...
 *  @return     0 if successful.
 */
int mpu_get_int_status(short *status, bool addr)
{
    if (addr)
        st.hw->addr = 0x69; 
        else
...
int mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
        unsigned char *sensors, unsigned char *more, bool addr)
{
    if (addr)
        st.hw->addr = 0x69; 
        else
 ...
 *  @return     0 if successful.
 */
int mpu_set_bypass(unsigned char bypass_on, bool addr)
{ 
    if (addr)
        st.hw->addr = 0x69; 
        else
 ...
 *  @return     0 if successful.
 */
int mpu_set_int_latched(unsigned char enable, bool addr)
{  if (addr)
        st.hw->addr = 0x69; 
        else
  st.hw->addr = 0x68;
...
#ifdef MPU6050
static int get_accel_prod_shift(float *st_shift, bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
static int accel_self_test(long *bias_regular, long *bias_st, bool addr)
{ 
if (addr)
        st.hw->addr = 0x69; 
else
  st.hw->addr = 0x68;
    ...
    get_accel_prod_shift(st_shift, addr);
    for(jj = 0; jj < 3; jj++) {
...
static int gyro_self_test(long *bias_regular, long *bias_st,bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
#endif 
#ifdef AK89xx_SECONDARY
static int compass_self_test(bool addr)
{
    unsigned char tmp[6];
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test,bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug,bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug, bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug, bool addr)
{
if (addr)
        st.hw->addr = 0x69; 
else
 ...
 */
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug, bool addr)
{ 
if (addr)
        st.hw->addr = 0x69; 
        else 
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_6500_biases(gyro, accel, 0, debug, addr))
            break;
    if (ii == tries) {
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_6500_biases(gyro_st, accel_st, 1, debug, addr))
            break;
    if (ii == tries) {
    ...
    accel_result = accel_6500_self_test(accel, accel_st, debug, addr);
    if(debug)
    	log_i("Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug, addr);
    if(debug)
...
restore:
	...
	st.chip_cfg.fifo_enable = 0xFF;
	st.chip_cfg.clk_src = INV_CLK_PLL;
	mpu_set_gyro_fsr(gyro_fsr,addr);
	mpu_set_accel_fsr(accel_fsr, addr);
	mpu_set_lpf(lpf, addr);
	mpu_set_sample_rate(sample_rate, addr);
	mpu_set_sensors(sensors_on, addr);
	mpu_configure_fifo(fifo_sensors, addr);

	if (dmp_was_on)
		mpu_set_dmp_state(1, addr);
 ...
 *  @return     Result mask (see above).
 */
int mpu_run_self_test(long *gyro, long *accel, bool addr)
{
#ifdef MPU6050
    const unsigned char tries = 2;
    long gyro_st[3], accel_st[3];
...
#if defined MPU6050
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(gyro, accel, 0,addr))
            break;
    if (ii == tries) {
        /* If we reach this point, we most likely encountered an I2C error.
         * We'll just report an error for all three sensors.
    ...
    for (ii = 0; ii < tries; ii++)
        if (!get_st_biases(gyro_st, accel_st, 1,addr))
            break;
    if (ii == tries) {
        /* Again, probably an I2C error. */
        result = 0;
    ...
    accel_result = accel_self_test(accel, accel_st, addr);
    gyro_result = gyro_self_test(gyro, gyro_st, addr );
...
restore:
     ...
     */
    get_st_biases(gyro, accel, 0, addr);
    result = 0x7;
#endif
    /* Set to invalid values to ensure no I2C writes are skipped. */
    st.chip_cfg.gyro_fsr = 0xFF;
...
int mpu_load_firmware(unsigned short length, const unsigned char *firmware,
    unsigned short start_addr, unsigned short sample_rate, bool addr)
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
#define LOAD_CHUNK  (16)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii], addr))
            return -1;
        if (mpu_read_mem(ii, this_write, cur, addr))
            return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_set_dmp_state(unsigned char enable, bool addr)
{ if (addr)
        st.hw->addr = 0x69; 
else 
 st.hw->addr = 0x68; 
    ...
    if (enable) {
        ...
        mpu_set_sample_rate(st.chip_cfg.dmp_sample_rate, addr);
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(short *data, unsigned long *timestamp, bool addr )
{
     if (addr)
        st.hw->addr = 0x69; 
 else
...
int mpu_lp_motion_interrupt(unsigned short thresh, unsigned char time,
    unsigned short lpa_freq, bool addr)
{
 if (addr)
        st.hw->addr = 0x69; 
        else 
...
lp_int_restore:
    ...
    st.chip_cfg.fifo_enable = 0xFF;
    st.chip_cfg.clk_src = INV_CLK_PLL;
    mpu_set_sensors(st.chip_cfg.cache.sensors_on, addr);
    mpu_set_gyro_fsr(st.chip_cfg.cache.gyro_fsr, addr);
    mpu_set_accel_fsr(st.chip_cfg.cache.accel_fsr, addr);
    mpu_set_lpf(st.chip_cfg.cache.lpf, addr);
    mpu_set_sample_rate(st.chip_cfg.cache.sample_rate, addr);
    mpu_configure_fifo(st.chip_cfg.cache.fifo_sensors, addr);

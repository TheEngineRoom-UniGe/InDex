...
static bool addr;
MPU9250_DMP::MPU9250_DMP(bool address)
{
...
inv_error_t MPU9250_DMP::begin()
{
	...
	result = mpu_init(&int_param,addr);
	
	if (result)
		return result;
	
	mpu_set_bypass(1,addr); // Place all slaves (including compass) on primary bus
...
{
	return set_int_enable(enable,addr);
}
...
{
	return mpu_set_int_latched(enable,addr);
}

short MPU9250_DMP::getIntStatus(void)
{
	short status;
	if (mpu_get_int_status(&status,addr) == INV_SUCCESS)
	{
		return status;
	}
	return 0;
...
{
	return mpu_lp_accel_mode(rate,addr);
}

inv_error_t MPU9250_DMP::setGyroFSR(unsigned short fsr)
{
	inv_error_t err;
	err = mpu_set_gyro_fsr(fsr,addr);
	if (err == INV_SUCCESS)
	{
		_gSense = getGyroSens();
	}
...
{
	inv_error_t err;
	err = mpu_set_accel_fsr(fsr, addr);
	if (err == INV_SUCCESS)
	{
		_aSense = getAccelSens();
	}
...
{
    return mpu_set_sample_rate(rate,addr);
}
...
{
	return mpu_set_compass_sample_rate(rate,addr);
}
...
{
	return mpu_configure_fifo(sensors,addr);
}
...
unsigned short MPU9250_DMP::fifoAvailable(void)
{
	...
	if (mpu_read_reg(MPU9250_FIFO_COUNTH, &fifoH,addr) != INV_SUCCESS)
		return 0;
	if (mpu_read_reg(MPU9250_FIFO_COUNTL, &fifoL,addr) != INV_SUCCESS)
		return 0;
...
inv_error_t MPU9250_DMP::updateFifo(void)
{
	...
	if (mpu_read_fifo(gyro, accel, &timestamp, &sensors, &more,addr) != INV_SUCCESS)
		return INV_ERROR;
...
{
	return mpu_set_sensors(sensors,addr);
}
...
bool MPU9250_DMP::dataReady()
{
	...
	if (mpu_read_reg(MPU9250_INT_STATUS, &intStatusReg,addr) == INV_SUCCESS)
	{
...
int MPU9250_DMP::updateAccel(void)
{
	...
	if (mpu_get_accel_reg(data, &time,addr))
	{
		return INV_ERROR;		
	}
	ax = data[X_AXIS];
...
int MPU9250_DMP::updateGyro(void)
{
	...
	if (mpu_get_gyro_reg(data, &time,addr))
	{
		return INV_ERROR;		
	}
	gx = data[X_AXIS];
...
int MPU9250_DMP::updateCompass(void)
{
	...
	if (mpu_get_compass_reg(data, &time,addr))
	{
		return INV_ERROR;		
	}
	mx = data[X_AXIS];
...
{
	return mpu_get_temperature(&temperature, &time,addr);
}

int MPU9250_DMP::selfTest(unsigned char debug)
{
	long gyro[3], accel[3];
	return mpu_run_self_test(gyro, accel,addr);
}
...
inv_error_t MPU9250_DMP::dmpBegin(unsigned short features, unsigned short fifoRate)
{
	...
	if (feat & DMP_FEATURE_LP_QUAT)
	{
		...
		dmp_enable_lp_quat(1,addr);
	}
	else if (feat & DMP_FEATURE_6X_LP_QUAT)
		dmp_enable_6x_lp_quat(1,addr);
	
	if (feat & DMP_FEATURE_GYRO_CAL)
		dmp_enable_gyro_cal(1,addr);
	...
	return mpu_set_dmp_state(1,addr);
}

inv_error_t MPU9250_DMP::dmpLoad(void)
{
	return dmp_load_motion_driver_firmware(addr);
}
...
inv_error_t MPU9250_DMP::dmpSetFifoRate(unsigned short rate)
{
	...
	return dmp_set_fifo_rate(rate,addr);
}
...
inv_error_t MPU9250_DMP::dmpUpdateFifo(void)
{
	...
	if (dmp_read_fifo(gyro, accel, quat, &timestamp, &sensors, &more, addr)
		   != INV_SUCCESS)
    {
	   return INV_ERROR;
    }
...
inv_error_t MPU9250_DMP::dmpEnableFeatures(unsigned short mask)
{
	...
	return dmp_enable_feature(enMask,addr);
}
...
inv_error_t MPU9250_DMP::dmpSetOrientation(const signed char * orientationMatrix)
{
	...
	return dmp_set_orientation(scalar, addr);
}
...
inv_error_t MPU9250_DMP::dmpEnable3Quat(void)
{
	...
	return dmp_enable_lp_quat(1,addr);
}

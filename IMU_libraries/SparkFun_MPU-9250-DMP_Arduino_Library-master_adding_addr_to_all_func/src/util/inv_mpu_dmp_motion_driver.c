...
{   
    ...
    if (mpu_write_mem(FCFG_1, 3, gyro_regs,addr))
        return -1;
    if (mpu_write_mem(FCFG_2, 3, accel_regs,addr))
        return -1;
    ...
    if (mpu_write_mem(FCFG_3, 3, gyro_regs,addr))
        return -1;
    if (mpu_write_mem(FCFG_7, 3, accel_regs,addr))
        return -1;
    dmp.orient = orient;
    return 0;
}
...
int dmp_set_gyro_bias(long *bias, bool addr)
{
  ...
  //       st.hw->addr = 0x68; 
    ...
    if (mpu_write_mem(D_EXT_GYRO_BIAS_X, 4, regs,addr))
        return -1;
    ...
    if (mpu_write_mem(D_EXT_GYRO_BIAS_Y, 4, regs,addr))
        return -1;
    ...
    return mpu_write_mem(D_EXT_GYRO_BIAS_Z, 4, regs,addr);
}
...
int dmp_set_accel_bias(long *bias, bool addr)
{
    ...
    return mpu_write_mem(D_ACCEL_BIAS, 12, regs, addr);
}
...
int dmp_set_fifo_rate(unsigned short rate, bool addr)
{
    ...
    if (mpu_write_mem(D_0_22, 2, tmp,addr))
        return -1;
    if (mpu_write_mem(CFG_6, 12, (unsigned char*)regs_end,addr))
        return -1;
...
int dmp_set_tap_thresh(unsigned char axis, unsigned short thresh)
{
    ...
    if (axis & TAP_X) {
        if (mpu_write_mem(DMP_TAP_THX, 2, tmp,0))
            return -1;
        if (mpu_write_mem(D_1_36, 2, tmp+2,0))
            return -1;
    }
    if (axis & TAP_Y) {
        if (mpu_write_mem(DMP_TAP_THY, 2, tmp,0))
            return -1;
        if (mpu_write_mem(D_1_40, 2, tmp+2,0))
            return -1;
    }
    if (axis & TAP_Z) {
        if (mpu_write_mem(DMP_TAP_THZ, 2, tmp,0))
            return -1;
        if (mpu_write_mem(D_1_44, 2, tmp+2,0))
            return -1;
    }
    return 0;
}
...
int dmp_set_tap_axes(unsigned char axis)
{
    ...
    if (axis & TAP_Z)
        tmp |= 0x03;
    return mpu_write_mem(D_1_72, 1, &tmp,0);
}
...
int dmp_set_tap_count(unsigned char min_taps)
{
    ...
    tmp = min_taps - 1;
    return mpu_write_mem(D_1_79, 1, &tmp, 0);
}
...
int dmp_set_tap_time(unsigned short time)
{
    ...
    return mpu_write_mem(DMP_TAPW_MIN, 2, tmp,0);
}
...
int dmp_set_tap_time_multi(unsigned short time)
{
    ...
    tmp[0] = (unsigned char)(dmp_time >> 8);
    tmp[1] = (unsigned char)(dmp_time & 0xFF);
    return mpu_write_mem(D_1_218, 2, tmp,0);
}
...
int dmp_set_shake_reject_thresh(long sf, unsigned short thresh)
{
    ...
    return mpu_write_mem(D_1_92, 4, tmp,0);
}
...
int dmp_set_shake_reject_time(unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem(D_1_90,2,tmp,0);
}
...
int dmp_set_shake_reject_timeout(unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem(D_1_88,2,tmp,0);
}
...
int dmp_get_pedometer_step_count(unsigned long *count)
{
    ...
    if (mpu_read_mem(D_PEDSTD_STEPCTR, 4, tmp, 0))
        return -1;
...
int dmp_set_pedometer_step_count(unsigned long count)
{
    ...
    return mpu_write_mem(D_PEDSTD_STEPCTR, 4, tmp,0);
}
...
int dmp_get_pedometer_walk_time(unsigned long *time)
{
    ...
    if (mpu_read_mem(D_PEDSTD_TIMECTR, 4, tmp, 0))
        return -1;
...
int dmp_set_pedometer_walk_time(unsigned long time)
{
    ...
    return mpu_write_mem(D_PEDSTD_TIMECTR, 4, tmp,0);
}
...
//  if (addr)
  //       st.hw->addr = 0x69; 
    ...
    mpu_write_mem(D_0_104, 4, tmp,addr);
    ...
    tmp[8] = 0xA3;
    tmp[9] = 0xA3;
    mpu_write_mem(CFG_15,10,tmp,addr);
    ...
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT))
        ...
        tmp[0] = 0xD8;
    mpu_write_mem(CFG_27,1,tmp,addr);

    if (mask & DMP_FEATURE_GYRO_CAL)
        dmp_enable_gyro_cal(1,addr);
    else
        dmp_enable_gyro_cal(0, addr);
    ...
    if (mask & DMP_FEATURE_SEND_ANY_GYRO) {
        ...
        mpu_write_mem(CFG_GYRO_RAW_DATA, 4, tmp, addr);
    }

    if (mask & DMP_FEATURE_TAP) {
        /* Enable tap. */
        tmp[0] = 0xF8;
        mpu_write_mem(CFG_20, 1, tmp, addr);
    ...
    if (mask & DMP_FEATURE_ANDROID_ORIENT) {
        ...
        tmp[0] = 0xD8;
    mpu_write_mem(CFG_ANDROID_ORIENT_INT, 1, tmp,addr);

    if (mask & DMP_FEATURE_LP_QUAT)
        dmp_enable_lp_quat(1, addr);
    else
       dmp_enable_lp_quat(0, addr);

    if (mask & DMP_FEATURE_6X_LP_QUAT)
        dmp_enable_6x_lp_quat(1,addr);
    else
        dmp_enable_6x_lp_quat(0,addr);
...
int dmp_enable_gyro_cal(unsigned char enable, bool addr)
{
    if (enable) {
        ...
        return mpu_write_mem(CFG_MOTION_BIAS, 9, regs, addr);
    } else {
...
int dmp_enable_lp_quat(unsigned char enable, bool addr)
{
 ...
 //        st.hw->addr = 0x68; 
    ...
    mpu_write_mem(CFG_LP_QUAT, 4, regs, addr);
...
int dmp_enable_6x_lp_quat(unsigned char enable, bool addr)
{
    ...
    mpu_write_mem(CFG_8, 4, regs,addr);
...
int dmp_set_interrupt_mode(unsigned char mode, bool addr)
{
    ...
    case DMP_INT_CONTINUOUS:
        return mpu_write_mem(CFG_FIFO_ON_EVENT, 11,
            (unsigned char*)regs_continuous, addr);
    case DMP_INT_GESTURE:
        return mpu_write_mem(CFG_FIFO_ON_EVENT, 11,
            (unsigned char*)regs_gesture, addr);
    default:
        return -1;
    }
}
    ...
    unsigned long *timestamp, short *sensors, unsigned char *more, bool addr)
{
    ...
    /* Get a packet. */
    if (mpu_read_fifo_stream(dmp.packet_length, fifo_data, more, addr))
        return -1;

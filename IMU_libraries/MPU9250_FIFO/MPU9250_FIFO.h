#pragma once
#ifndef MPU9250_FIFO_H
#define MPU9250_FIFO_H
...
#include "MPU9250_FIFO/MPU9250RegisterMap.h"
#include "MPU9250_FIFO/QuaternionFilter.h"
#include <EEPROM.h>
#define SLAVE true
...
class MPU9250_FIFO_
{
    uint8_t MPU9250_ADDRESS {0x68};  // Device address when ADO = 0
    ...
    /// EEPROM variables
	const uint8_t EEPROM_SIZE = 1 + sizeof(float) * 3 * 4; // single calibration byte size

bool b_calibrated = false;
uint8_t index =0; // the sensor number; 
uint8_t offset = 0; 
enum EEP_ADDR
{
    EEP_CALIB_FLAG = 0x00,
...
public:
    ...
    MPU9250_FIFO_() : aRes(getAres()), gRes(getGres()), mRes(getMres()) {}
    MPU9250_FIFO_(uint8_t index) : aRes(getAres()), gRes(getGres()), mRes(getMres()) {this->index = index; offset = index * EEPROM_SIZE;}
  ...
  // Configure device for bias calculation
        writeByte(MPU9250_ADDRESS, INT_ENABLE, 0x01);   // Disable all interrupts
...
////// 
    ...
    {
        calibrateMPU9250(gyroBias, accelBias,SLAVE);
    }

    void calibrateMag()
    {
		
        magcalMPU9250(magBias, magScale,SLAVE);
    }
    ...
    bool isConnectedAK8963() // todo modify
    {
    ...
    bool available()
    {
		...
		// the mag data is available at 100 Hz (depending on the mode)
    }

    void update()
    {
        if (available()) // we should add condition to mag
        {  // On interrupt, check if data ready interrupt
		   //Serial.println("updating Gyro"); 
		   updateAccelGyro();
        //    Serial.println("updating Mag"); 
			//updateMag(); // TODO: set to 30fps?
			updateMagFIFO();
		//	Serial.println("done mag");
        }
...
//    void setI2CAddress(uint8_t addr) {} // TODO:
    ...
    {
        Serial.println("< calibration parameters local variables>");
...
{   
    EEPROM.writeByte(EEP_CALIB_FLAG + offset, 1);
    EEPROM.writeFloat(EEP_ACC_BIAS + offset + 0, getAccBias(0));
    EEPROM.writeFloat(EEP_ACC_BIAS + offset + 4, getAccBias(1));
    EEPROM.writeFloat(EEP_ACC_BIAS + offset + 8, getAccBias(2));
    EEPROM.writeFloat(EEP_GYRO_BIAS + offset + 0, getGyroBias(0));
    EEPROM.writeFloat(EEP_GYRO_BIAS + offset + 4, getGyroBias(1));
    EEPROM.writeFloat(EEP_GYRO_BIAS + offset + 8, getGyroBias(2));
    EEPROM.writeFloat(EEP_MAG_BIAS + offset + 0, getMagBias(0));
    EEPROM.writeFloat(EEP_MAG_BIAS + offset + 4, getMagBias(1));
    EEPROM.writeFloat(EEP_MAG_BIAS + offset + 8, getMagBias(2));
    EEPROM.writeFloat(EEP_MAG_SCALE + offset + 0, getMagScale(0));
    EEPROM.writeFloat(EEP_MAG_SCALE + offset + 4, getMagScale(1));
    EEPROM.writeFloat(EEP_MAG_SCALE + offset + 8, getMagScale(2));
...
void loadCalibration()
{
    ...
    {   Serial.print(index);
        ...
        setAccBias(0, EEPROM.readFloat(EEP_ACC_BIAS + offset + 0));
        setAccBias(1, EEPROM.readFloat(EEP_ACC_BIAS + offset + 4));
        setAccBias(2, EEPROM.readFloat(EEP_ACC_BIAS + offset + 8));
        setGyroBias(0, EEPROM.readFloat(EEP_GYRO_BIAS + offset + 0));
        setGyroBias(1, EEPROM.readFloat(EEP_GYRO_BIAS + offset + 4));
        setGyroBias(2, EEPROM.readFloat(EEP_GYRO_BIAS + offset + 8));
        setMagBias(0, EEPROM.readFloat(EEP_MAG_BIAS + offset + 0));
        setMagBias(1, EEPROM.readFloat(EEP_MAG_BIAS + offset+ 4));
        setMagBias(2, EEPROM.readFloat(EEP_MAG_BIAS + offset + 8));
        setMagScale(0, EEPROM.readFloat(EEP_MAG_SCALE + offset + 0));
        setMagScale(1, EEPROM.readFloat(EEP_MAG_SCALE + offset + 4));
        setMagScale(2, EEPROM.readFloat(EEP_MAG_SCALE + offset+ 8));
    }
    else
    {
...
{
    for (size_t i = 0 + offset ; i < EEPROM_SIZE + offset; ++i)
...
bool isCalibrated()
{
    return (EEPROM.readByte(EEP_CALIB_FLAG + offset) == 0x01);
}


void printCalibrationfromEEPROM()
{
    Serial.println("< calibration parameters from EEPROM sensor index >");
	Serial.print(index);
    Serial.print("calibrated? : ");
    Serial.println(isCalibrated() ? "YES" : "NO");
    Serial.print("acc bias x  : ");
    Serial.println(EEPROM.readFloat(EEP_ACC_BIAS + offset + 0) * 1000.f);
    Serial.print("acc bias y  : ");
    Serial.println(EEPROM.readFloat(EEP_ACC_BIAS + offset + 4) * 1000.f);
    Serial.print("acc bias z  : ");
    Serial.println(EEPROM.readFloat(EEP_ACC_BIAS + offset + 8) * 1000.f);
    Serial.print("gyro bias x : ");
    Serial.println(EEPROM.readFloat(EEP_GYRO_BIAS + offset + 0));
    Serial.print("gyro bias y : ");
    Serial.println(EEPROM.readFloat(EEP_GYRO_BIAS + offset + 4));
    Serial.print("gyro bias z : ");
    Serial.println(EEPROM.readFloat(EEP_GYRO_BIAS + offset + 8));
    Serial.print("mag bias x  : ");
    Serial.println(EEPROM.readFloat(EEP_MAG_BIAS + offset + 0));
    Serial.print("mag bias y  : ");
    Serial.println(EEPROM.readFloat(EEP_MAG_BIAS + offset + 4));
    Serial.print("mag bias z  : ");
    Serial.println(EEPROM.readFloat(EEP_MAG_BIAS + offset + 8));
    Serial.print("mag scale x : ");
    Serial.println(EEPROM.readFloat(EEP_MAG_SCALE + offset + 0));
    Serial.print("mag scale y : ");
    Serial.println(EEPROM.readFloat(EEP_MAG_SCALE + offset + 4));
    Serial.print("mag scale z : ");
    Serial.println(EEPROM.readFloat(EEP_MAG_SCALE + offset + 8));
}

void lazyCalibration(uint8_t n_sensors)
{
	if (!isCalibrated() || (index !=0)) 
	{
	Serial.println (" Sensor 0 should be calibrated, and method should be called from sensor 0");
	...
	{
	int offset2 = i * EEPROM_SIZE;
	EEPROM.writeByte(EEP_CALIB_FLAG + offset2, 1);
    EEPROM.writeFloat(EEP_ACC_BIAS + offset2 + 0, getAccBias(0));
    EEPROM.writeFloat(EEP_ACC_BIAS + offset2 + 4, getAccBias(1));
    EEPROM.writeFloat(EEP_ACC_BIAS + offset2 + 8, getAccBias(2));
    EEPROM.writeFloat(EEP_GYRO_BIAS + offset2 + 0, getGyroBias(0));
    EEPROM.writeFloat(EEP_GYRO_BIAS + offset2 + 4, getGyroBias(1));
    EEPROM.writeFloat(EEP_GYRO_BIAS + offset2 + 8, getGyroBias(2));
    EEPROM.writeFloat(EEP_MAG_BIAS + offset2 + 0, getMagBias(0));
    EEPROM.writeFloat(EEP_MAG_BIAS + offset2 + 4, getMagBias(1));
    EEPROM.writeFloat(EEP_MAG_BIAS + offset2 + 8, getMagBias(2));
    EEPROM.writeFloat(EEP_MAG_SCALE + offset2 + 0, getMagScale(0));
    EEPROM.writeFloat(EEP_MAG_SCALE + offset2 + 4, getMagScale(1));
    EEPROM.writeFloat(EEP_MAG_SCALE + offset2 + 8, getMagScale(2));
    EEPROM.commit();	
	}
}
...
void setupEEPROM() // It won't be usefull like this
{
    Serial.println("EEPROM start");
    if (!EEPROM.begin(512))//(EEPROM_SIZE))
    {
...
void updateMagFIFO()
    {
        int16_t magCount[3] = {0, 0, 0};    // Stores the 16-bit signed magnetometer sensor output
        readMagDataFIFO(magCount);  // Read the x/y/z adc values
        // getMres();
   ...
   void readMagDataFIFO(int16_t * destination)
    {
	        uint8_t rawData[7];  // x/y/z gyro register data, ST2 register stored here, must read ST2 at end of data acquisition
    
    // if(readByte(AK8963_ADDRESS, AK8963_ST1) & 0x01) { // wait for magnetometer data ready bit to be set
uint8_t trials = 0;	
while(!(readByte(MPU9250_ADDRESS, EXT_SENS_DATA_00) & 0x01)) ; // sometimes it tooks 15 times to get the data_ready flag
	//{Serial.println("trial ");Serial.print(trials++);Serial.print ("##");}
//; //TODO this was added because sometimes the ready flag is not set to ONE but this creates extra delay
	//if(readByte(MPU9250_ADDRESS, EXT_SENS_DATA_00) & 0x01)
	{ // EXT_SENS_DATA_00 CONTAINS AK8963_ST1
	 //readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
	readBytes(MPU9250_ADDRESS, EXT_SENS_DATA_01, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
             uint8_t c = rawData[6]; // End data read by reading ST2 register
			// Serial.print(" ST mag ");Serial.println(c);
             if(!(c & 0x08) ) { // Check if magnetic sensor overflow set, if not then report data
             destination[0] = ((int16_t)rawData[1] << 8) | rawData[0];  // Turn the MSB and LSB into a signed 16-bit value
             destination[1] = ((int16_t)rawData[3] << 8) | rawData[2];  // Data stored as little Endian
             destination[2] = ((int16_t)rawData[5] << 8) | rawData[4];
		 ...
		 // else 
		 // {
		  // Serial.print(" EXT_SENS_DATA_00 = ");
		  // Serial.println (readByte(MPU9250_ADDRESS, EXT_SENS_DATA_00));
		  // Serial.print(" EXT_SENS_DATA_07 = ");
		  // Serial.println (readByte(MPU9250_ADDRESS, EXT_SENS_DATA_07));
		 		 // }
    }
    void updateRPY()
    {
    ...
    void initAK8963(float * destination)
    {
        ...
        writeByte(AK8963_ADDRESS, AK8963_CNTL, (uint8_t)MFSSEL << 4 | Mmode); // Set magnetometer data resolution and sample ODR // MMODE 1111
        delay(10);
	...
	void initAK8963FIFO()
	{
	writeByte(MPU9250_ADDRESS, USER_CTRL, 0x60); // enable fifo and master mode , choose slv0
	...
	writeByte(MPU9250_ADDRESS, I2C_SLV0_REG, AK8963_ST1);  // data ready is stored at AK8963_ST1 (0x02) // read dataready, and registers
    writeByte(MPU9250_ADDRESS, I2C_MST_CTRL, 0x4D);  // 0xXD =13 --> 400khz  // 0x4X ==> wait for External sensor data

    writeByte(MPU9250_ADDRESS, I2C_SLV0_CTRL, 0x88); // 0x8X for svl0en , 0xX8 read 8 bytes
//    writeByte(MPU9250_ADDRESS, USER_CTRL, 0x0C);    // Reset FIFO and DMP

	}
	
    void magcalMPU9250(float * dest1, float * dest2, bool isMagSlave)
    {
    ...
    void calibrateMPU9250(float * dest1, float * dest2, bool isSlave)
    {
	...
	void writeBytetoSlave(uint8_t address, uint8_t register, uint8_t data)
	{
		// FIFO and master mode should be enabled 
	   ...
	   writeByte(MPU9250_ADDRESS, I2C_SLV0_ADDR, 0x0C); // address 0x0C & 0x0X for writeread  

		writeByte(MPU9250_ADDRESS, I2C_SLV0_REG, data); 
    ...
    uint8_t readByte(uint8_t address, uint8_t subAddress)
    {
        ...
        wire->beginTransmission(address);         // Initialize the Tx buffer
        wire->write(subAddress);	                 // Put slave register address in Tx buffer
//		Serial.print("subbaddress: ");
//		Serial.print(subAddress);
        i2c_err_ = wire->endTransmission(false); //        // Send the Tx buffer, but send a restart to keep connection alive // putting it to false returns error on reading interrupt status
        if (i2c_err_) {Serial.print ("read byte ");Serial.print(address);Serial.print(" sub :"); Serial.print(subAddress);printI2CError();}
        wire->requestFrom(address, (size_t)1);  // Read one byte from slave register address
        if (wire->available()) data = wire->read();                      // Fill Rx buffer with result
		// i2c_err_ = wire->endTransmission();        // TODO added to solve the  latency problem // adding this produced I2C_err_code 8 // and did not help
    ...
    void readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest)
    {
        ...
        if (i2c_err_) {Serial.print(address);printI2CError();}
        uint8_t i = 0;
...
#ifdef TEENSYDUINO
using MPU9250_FIFO = MPU9250_FIFO_<i2c_t3>;
#else
using MPU9250_FIFO = MPU9250_FIFO_<TwoWire>;
#endif
